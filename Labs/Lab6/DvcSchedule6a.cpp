...
ostream& operator<<(ostream& output, const SubjectCode& s) 
{
    output.width(6); output << left << s.name << tab << tab << s.count;
 ...
 // read the input file
    {
      ...
      string subjectCode(course.begin(), course.begin() + course.find('-'));  cout << tab << " read in file " << endl;
